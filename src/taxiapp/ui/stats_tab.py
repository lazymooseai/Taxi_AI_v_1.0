...
def render_learning_section(
    ...
    with st.expander("Lisaa opetusdata kuvasta tai tiedostosta", expanded=False):
        st.caption(
            "Ota kuva valituslistan kuvaruudusta tai lataa txt-tiedosto. "
            "Teksti tunnistetaan automaattisesti (OCR) ja malli oppii siita."
        )

        input_tab1, input_tab2 = st.tabs(["Kamera", "Tiedosto"])
        ...
        with input_tab1:
            ...
            if img_file:
                try:
                    ...
                    ocr_text = "\n".join(results)
                    st.success(f"Tunnistettu {len(results)} tekstilohkoa")
                    with st.expander("Tunnistettu teksti", expanded=False):
                        st.text(ocr_text[:1000])
                except Exception as e:
                    st.error(f"OCR-virhe: {e}")

        with input_tab2:
            up_file = st.file_uploader(
                "Lataa tiedosto (txt, csv, kuva)",
                type=["txt", "csv", "png", "jpg", "jpeg"],
                key="teach_file_upload",
            )
            if up_file:
                ...
                try:
                    if fname.endswith((".png", ".jpg", ".jpeg")):
                        ...
                        results = reader.readtext(img_array, detail=0, paragraph=True)
                        ocr_text = "\n".join(results)
                        st.success(f"Kuva luettu: {len(results)} lohkoa")
                    else:
                        raw = up_file.read()
                        ocr_text = raw.decode("utf-8", errors="ignore")
                        st.success(f"Tiedosto luettu: {len(ocr_text)} merkkkia")
                    with st.expander("Luettu sisalto", expanded=False):
                        st.text(ocr_text[:1000] if ocr_text else "")
                except Exception as e:
                    st.error(f"Luku-virhe: {e}")

        if ocr_text:
            st.divider()
            st.caption("Kasittele ja tallenna Supabaseen:")
            ocr_area = st.text_input(
                "Alue jolle data liittyy",
                value="Rautatieasema",
                key="ocr_area",
            )
            if st.button("Tallenna Supabaseen", key="btn_save_ocr"):
                try:
                    from src.taxiapp.repository.database import get_db
                    import json as _json
                    now_iso = datetime.now(timezone.utc).isoformat()
                    get_db().table("dispatch_preorders").insert({
                        "captured_at":           now_iso,
                        "driver_location_name":  ocr_area,
                        "raw_ocr_text":          ocr_text[:4000],
                        "row_type":              "ocr_import",
                    }).execute()
                    st.success("Tallennettu Supabaseen!")
                except Exception as e:
                    st.warning(f"Tallennus epaonnistui: {e}")
