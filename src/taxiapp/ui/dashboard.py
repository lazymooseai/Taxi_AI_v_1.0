...
def render_news(agent_results: list[AgentResult]) -> None:
    ...
    for item in news_items[:5]:
        ...
        urg_icon    = "" if urgency >= 7 else ("" if urgency >= 5 else "")
...
def render_weather_detail(weather_raw: dict) -> None:
    ...
    with col1:
        st.metric(" Lämpötila", temp_str)
    with col2:
        st.metric(" Tuuli", wind_str, gust_str if gust_str else None)
    with col3:
        st.metric(" Sade", prec_str)
    with col4:
        st.metric(" Näkyvyys", vis_str)
...
def render_dashboard() -> None:
    ...
    if weather_raw:
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        with st.expander(" Sää yksityiskohtaisesti", expanded=False):
