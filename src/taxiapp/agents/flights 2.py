...
Hakee saapuvat lennot Helsinki-Vantaan lentokentältä (EFHK).
...
class FlightAgent(BaseAgent):
    ...
    def _flight_to_signal(
        ...
        # Peruspisteet matkustajamäärän mukaan
        score_base = max(5.0, pax / 10.0)

        # Tikkurila saa pienemmän boostin
        ...
        if delay >= 60:
            ...
            delay_bonus = 30.0
            delay_reason = (
                f"✈️ {flight.flight_no} MYÖHÄSSÄ {delay}min "
            ...
            delay_bonus = 8.0
            delay_reason = (
                f"✈️ {flight.flight_no} +{delay}min "
                f"({flight.origin_city or flight.origin})"
            )
        ...
        if 0 <= eta <= 10:
            ...
            eta_reason = (
                f"✈️ {flight.flight_no} laskeutuu ~{max(0, int(eta))}min "
                f"({flight.origin_city or flight.origin})"
            )
        elif 10 < eta <= 30:
            ...
            eta_reason = (
                f"✈️ {flight.flight_no} saapuu {int(eta)}min päästä "
                f"({flight.origin_city or flight.origin})"
            )
        elif 30 < eta <= 60:
            eta_urgency = 3
            eta_score = score_base
            eta_reason = f"✈️ {flight.flight_no} saapuu {int(eta)}min päästä"
        else:
            eta_urgency = 2
...
def _parse_finavia_html(html: str) -> list[FlightArrival]:
    ...
    if not json_blocks:
        # Fallback: etsi HTML-taulukon rivit
        rows = re.findall(
            r'<tr[^>]*>\s*<td>([^<]*)</td>\s*<td>([^<]*)</td>',
            html, re.DOTALL
        )
        if rows:
            for flight_no, origin in rows[:MAX_FLIGHTS]:
                try:
                    f = FlightArrival(
...
def _parse_dt_flex(date_str: str) -> Optional[datetime]:
    """Jäsennä fleksibeli datetime-stringi UTC:ksi."""
...
def _dedup_signals(signals: list[Signal]) -> list[Signal]:
    """Deduplicoi signaalit samalta alueelta (säilytä korkein urgency)."""
    seen: dict[str, Signal] = {}

    for sig in signals:
        key = sig.area
        if key not in seen or sig.urgency > seen[key].urgency:
            seen[key] = sig
